...
data=$(echo $data | apertium-destxt)
...
tagger_output=$(echo $tagger_output | sed 's/+o seu\([^$]*\$\)/+o\1 ^seu\1/g') # Divide "o seu" into two tokens
tagger_output=$(echo $tagger_output | sed 's/+o qual\([^$]*\$\)/+o\1 ^qual\1/g') # Divide "o seu" into two tokens
tagger_output=$(echo $tagger_output | sed 's/+o nosso\([^$]*\$\)/+o\1 ^nosso\1/g') # Divide "o seu" into two tokens
echo $tagger_output

# TODO: LIDAR COM PORCENTAGEM
